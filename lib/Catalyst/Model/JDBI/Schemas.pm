package Catalyst::Model::JDBI::Schemas;
...
sub new {
  ...
  croak "->config->{schema_base} must be defined for this model"
     unless $self->schema_base;
  ...
  foreach my $moniker ( findsubmod $self->{schema_base} ) {
...
sub _select_name {
  ...
  return ( sort keys %{ $self->connect_infos } )[0];
}
...
sub databases {
  my $self = shift;
  return keys %{ $self->connect_infos };
}
...
L<Jifty::DBI>, L<Catalyst::Model::Jifty::DBI>
